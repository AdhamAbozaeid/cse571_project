...
0. Run ./env_setup.sh
1. Run roscore
2. Run rosrun cse571_project server.py -d 6 -n 40 -f 5 -b 5
3. Run roslaunch cse571_project maze.launch
4. Enter desired goal position in terminal running server.py (if running with -default-goal 0)
5. Run rosrun cse571_project move_tbot3.py
6. Run rosrun cse571_project search.py -a astar -c
server.py
Implements action execution and environment updation functions
Command-line arguments for server.py:
-h, --help show this help message and exit -d 5 for providing dimension of the grid -n 15 for providing no. of obstacles to be added in the grid -s 32 for providing random seed -f 3 for providing no. of refueling stations -b 15 for providing initial battery level -hdl 0 for running in headless mode(0 or 1) -default-goal 0 for running with default goal 
Note:
